...
void event(const char * data, size_t length) {
  ...
  Serial.println(data);
    ...
    if(data_from_server["type"] == "pointing_left") {
      ...
      Serial.print("pointing_left : ");Serial.print(angleX); Serial.print(" , ");Serial.println(angleY);
      ...
      Serial.print("pointing_right : ");Serial.print(angleX); Serial.print(" , ");Serial.println(angleY);
...
//BTN RIGHT
    ...
    //hand_neopixerl
  //  if (strlen(data_from_server["order"])==12) { // 데이터 길이 확인

  //     for (int i = 0; i < 12; i++) {
  //       if (data_from_server["order"][i] == '1') {
  //         control(i + 1); // control 함수 호출
  //         Serial.print("if started, order ");
  //         Serial.println(i + 1);
  //       }
  //     }delay(500);
  //   }
